...
<script type="text/javascript">
	...
	$(function() {
		...
		// 	   }else{
		// 		//提醒输入的密码要一致
		// 		$.messager.alert("提示信息","两次输入额密码不一致","warning")
        //
		// 	   }
        //
		// 	}
        ...
        $("#btnEp").click(function () {
            alert("111");
            //对表单进行校验
            var  c=$("#editPasswordFrom").form("validate")
            alert(c)
            //如果提交成功
            if(c){
                //手动校验两次的输入是否一次
                var  v=$("#txtNewPass").val();
                var  v2=$("#txtRePass").val();
                if(v==v2){
                    //发送ajax请求
                 $.post("userAction_editPassword.action",{"passwword":v},function (data) {
                     //修改成功
                     if(data == '1'){
                         //修改成功，关闭修改密码窗口
                         $("#editPwdWindow").window("close");
                ...
                //如果输入的情况有误，则给出提示的信息
                $.messager.alert("提示信息","两次输入的密码不正确","warning")
    ...
    <div id="editPwdWindow" class="easyui-window" title="修改密码" collapsible="false" minimizable="false" modal="true" closed="true" resizable="false"
        ...
        <div class="easyui-layout" fit="true">
            ...
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
				<%--使用form表单进行表单的校验--%>
				<form id="editPasswordFrom" method="post">
                <table cellpadding=3>
                    <tr>
                        <td>新密码：</td>
